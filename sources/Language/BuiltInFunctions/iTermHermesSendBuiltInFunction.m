//
//  iTermHermesSendBuiltInFunction.m
//  iTerm2SharedARC
//
//  (Fork) See iTermHermesSendBuiltInFunction.h.
//
...
@implementation iTermHermesSendBuiltInFunction

+ (void)registerBuiltInFunction {
    iTermBuiltInFunction *func =
    [[iTermBuiltInFunction alloc] initWithName:@"hermes_send"
                        ...
                        sideEffectsPlaceholder:@"[hermes_send]"
                                         block:
...
+ (NSError *)errorWithMessage:(NSString *)message {
    return [NSError errorWithDomain:@"com.copypastalabs.iterm2.hermes"
                               code:1
...
+ (void)sendMessage:(id)message
    ...
    if (![message isKindOfClass:[NSString class]] || [(NSString *)message length] == 0) {
        completion(nil, [self errorWithMessage:@"hermes_send requires a non-empty message"]);
        return;
    }
    ...
    [session sendHermesMessage:(NSString *)message
